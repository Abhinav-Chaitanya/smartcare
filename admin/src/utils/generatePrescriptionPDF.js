...
export const generatePrescriptionPDF = async (appointment, logoUrl) => {
    ...
    // Colors
    const primaryColor = [95, 111, 255]  // #5f6FFF
    ...
    try {
        ...
        // If logo fails to load, add text instead
        doc.setFontSize(24)
        doc.setTextColor(...primaryColor)
        doc.setFont('helvetica', 'bold')
        doc.text('SmartCare', pageWidth / 2, yPos + 10, { align: 'center' })
        yPos += 15
    }
    ...
    doc.setTextColor(...darkColor)
    doc.setFont('helvetica', 'bold')
    doc.text(`Name: `, leftCol, boxStartY + 8)
    doc.setFont('helvetica', 'normal')
    doc.text(appointment.userData?.name || 'N/A', leftCol + 15, boxStartY + 8)

    doc.setFont('helvetica', 'bold')
    doc.text(`Email: `, leftCol, boxStartY + 15)
    doc.setFont('helvetica', 'normal')
    doc.text(appointment.userData?.email || 'N/A', leftCol + 15, boxStartY + 15)

    doc.setFont('helvetica', 'bold')
    doc.text(`Phone: `, leftCol, boxStartY + 22)
    doc.setFont('helvetica', 'normal')
    doc.text(appointment.userData?.phone || 'N/A', leftCol + 15, boxStartY + 22)
    ...
    doc.setFont('helvetica', 'bold')
    doc.text(`Age: `, leftCol, boxStartY + 29)
    doc.setFont('helvetica', 'normal')
    doc.text(calculateAge(appointment.userData?.dob), leftCol + 12, boxStartY + 29)

    doc.setFont('helvetica', 'bold')
    doc.text(`Gender: `, leftCol, boxStartY + 36)
    doc.setFont('helvetica', 'normal')
    const gender = appointment.userData?.gender || 'N/A'
    doc.text(gender.charAt(0).toUpperCase() + gender.slice(1), leftCol + 18, boxStartY + 36)
    ...
    doc.setTextColor(...darkColor)
    doc.setFont('helvetica', 'bold')
    doc.text(`Doctor: `, rightCol, boxStartY + 8)
    doc.setFont('helvetica', 'normal')
    doc.text(`Dr. ${appointment.docData?.name || 'N/A'}`, rightCol + 17, boxStartY + 8)

    doc.setFont('helvetica', 'bold')
    doc.text(`Speciality: `, rightCol, boxStartY + 15)
    doc.setFont('helvetica', 'normal')
    doc.text(appointment.docData?.speciality || 'N/A', rightCol + 22, boxStartY + 15)
    ...
    doc.setFont('helvetica', 'bold')
    doc.text(`Date: `, rightCol, boxStartY + 22)
    doc.setFont('helvetica', 'normal')
    doc.text(formatSlotDate(appointment.slotDate), rightCol + 13, boxStartY + 22)

    doc.setFont('helvetica', 'bold')
    doc.text(`Time: `, rightCol, boxStartY + 29)
    doc.setFont('helvetica', 'normal')
    doc.text(appointment.slotTime || 'N/A', rightCol + 13, boxStartY + 29)

    doc.setFont('helvetica', 'bold')
    doc.text(`Fees: `, rightCol, boxStartY + 36)
    doc.setFont('helvetica', 'normal')
    doc.text(`Rs. ${appointment.amount || 'N/A'}`, rightCol + 13, boxStartY + 36)

    yPos += 58

    // Status badge
    doc.setFillColor(...greenColor)
    doc.roundedRect(pageWidth - margin - 30, yPos - 50, 25, 8, 2, 2, 'F')
    doc.setFontSize(8)
    doc.setTextColor(255, 255, 255)
    doc.setFont('helvetica', 'bold')
    doc.text('COMPLETED', pageWidth - margin - 28, yPos - 44.5)
    ...
    doc.setFont('helvetica', 'bold')
    doc.text('DIAGNOSIS', margin + 10, yPos + 10)
    ...
    doc.text(diagnosisLines, margin + 10, yPos + 18)
    
    yPos += 30 + (diagnosisLines.length > 1 ? (diagnosisLines.length - 1) * 5 : 0)

    // ==================== PRESCRIPTION SECTION ====================
    doc.setFontSize(12)
    doc.setTextColor(...primaryColor)
    doc.setFont('helvetica', 'bold')
    doc.text('PRESCRIPTION', margin, yPos + 5)
    yPos += 12
    ...
    if (appointment.prescription?.hasMedicines && appointment.prescription.medicines?.length > 0) {
        ...
        const colWidths = [10, 60, 30, 35, 35]
        let xPos = margin
        ...
        doc.rect(margin, yPos - 10 * appointment.prescription.medicines.length - 12, pageWidth - 2 * margin, 10 * appointment.prescription.medicines.length + 12)
        ...
        doc.text('No medicines prescribed', pageWidth / 2, yPos + 10, { align: 'center' })
        yPos += 20
    }
    ...
    if (appointment.prescription?.notes) {
        ...
        const notesLines = doc.splitTextToSize(appointment.prescription.notes, pageWidth - 2 * margin - 20)
        const notesHeight = 15 + notesLines.length * 5
        doc.roundedRect(margin, yPos, pageWidth - 2 * margin, notesHeight, 3, 3, 'F')

        doc.setFontSize(10)
        doc.setTextColor(146, 64, 14) // Amber color
        doc.setFont('helvetica', 'bold')
        doc.text("DOCTOR'S INSTRUCTIONS:", margin + 10, yPos + 10)

        doc.setFont('helvetica', 'normal')
        doc.text(notesLines, margin + 10, yPos + 18)
    ...
    doc.setTextColor(...grayColor)
    doc.setFont('helvetica', 'normal')
    doc.text("Doctor's Signature", pageWidth - margin - 30, yPos + 6, { align: 'center' })
    ...
    doc.setFontSize(8)
    doc.setTextColor(...grayColor)
    doc.text('This is a computer-generated prescription from SmartCare Hospitals.', pageWidth / 2, yPos + 6, { align: 'center' })
    doc.text(`Generated on: ${new Date().toLocaleString('en-IN')}`, pageWidth / 2, yPos + 11, { align: 'center' })
...
export const generatePrescriptionPDF = async (appointment, logoUrl) => {
    ...
    const diagnosis = appointment.diagnosis || 'No diagnosis recorded'
    const diagnosisLines = doc.splitTextToSize(diagnosis, pageWidth - 2 * margin - 20)
    const diagnosisHeight = 12 + (diagnosisLines.length * 4)
    ...
    doc.setTextColor(...darkColor)
    doc.setFont('helvetica', 'normal')
    doc.text(diagnosisLines, margin + 8, yPos + 12)

    yPos += diagnosisHeight + 5
    ...
    if (appointment.prescription?.notes) {
        ...
        const notesHeight = 10 + notesLines.length * 4
        ...
        doc.setFont('helvetica', 'normal')
        doc.text(notesLines, margin + 8, yPos + 11)
        
        yPos += notesHeight + 5
    }

    // ==================== SIGNATURE SECTION ====================
    // Calculate remaining space and position signature appropriately
    const footerHeight = 25
    const signatureY = Math.min(yPos + 10, pageHeight - footerHeight - 20)
