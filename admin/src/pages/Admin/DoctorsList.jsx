    ...
    <div className='m-5 max-h-[90vh] overflow-y-scroll'>
      ...
      <div className='w-full flex flex-wrap gap-4 pt-5 gap-y-6'>
      {
        doctors.map((item,index)=>(
          <div className='border border-indigo-200 rounded-xl max-w-56 overflow-hidden cursor-pointer group hover:-translate-y-2.5 transition-all duration-500' key={index}>
...
const DoctorsList = () => {
    ...
    // ✅ NEW: Get department from URL params
    ...
    // Filter doctors based on department (from URL) and search term
    const getFilteredDoctors = () => {
        let filtered = [...doctors]

        // Apply department filter if coming from specific department
    ...
    return (
        <div className='w-full max-w-6xl m-5'>
            ...
            <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6'>
                ...
                <button
                    onClick={() => {
                        // ✅ Navigate with department pre-selected if on specific department
                        if (decodedDepartment) {
                ...
                <div className='flex flex-col lg:flex-row gap-4 justify-between'>
                    ...
                    {decodedDepartment && (
                        <div className='flex items-center gap-2'>
                            <span className='px-4 py-2 bg-primary/10 text-primary rounded-lg font-medium flex items-center gap-2'>
                                <svg className='w-4 h-4' fill='none' stroke='currentColor' viewBox='0 0 24 24'>
                                    <path strokeLinecap='round' strokeLinejoin='round' strokeWidth='2' d='M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4' />
                                </svg>
                                {decodedDepartment}
            ...
            <div className='mb-4'>
                <p className='text-sm text-gray-500'>
                    ...
                    {decodedDepartment && (
                        <span> in <span className='font-semibold text-primary'>{decodedDepartment}</span> department</span>
                    )}
                </p>
            </div>
                ...
                <div className='bg-white rounded-xl shadow-sm p-12 text-center'>
                    ...
                    <p className='text-gray-500 mb-4'>
                        {searchTerm 
                            ...
                            : decodedDepartment 
                                ? `No doctors in ${decodedDepartment} department yet`
                                : 'No doctors registered yet'
                        }
                    </p>
                    {decodedDepartment && (
                        <button
                            onClick={() => navigate(`/add-doctor/${encodeURIComponent(decodedDepartment)}`)}
                            className='px-6 py-2.5 bg-primary text-white rounded-xl font-semibold hover:bg-primary/90 transition-all'
                        >
                            Add Doctor to {decodedDepartment}
                        </button>
                    )}
                </div>
            ) : (
                <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5'>
                    {filteredDoctors.map((item, index) => (
                        ...
                        >
                            ...
                            <div className='relative bg-gradient-to-br from-primary/10 to-blue-50 p-6 flex justify-center'>
                                <img
                                ...
                                <div className={`absolute top-3 right-3 flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${item.available
