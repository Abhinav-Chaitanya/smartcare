...
const AppointmentDetails = () => {
    ...
    useEffect(() => {
        const fetchAppointment = async () => {
            try {
                ...
                if (data.success) {
                    const found = data.appointments.find(apt => apt._id === appointmentId)
    ...
    const { docData, slotDate, slotTime, status, diagnosis, prescription, completedAt, amount } = appointment

    // Check if appointment receipt can be downloaded (confirmed or completed)
    //const canDownloadReceipt = status === 'confirmed' || status === 'completed'
    // Allow download for all appointment statuses
                ...
                <div className='p-6'>
                    <div className='flex items-start gap-6 flex-wrap'>
                        <img
                            src={docData.image}
                            alt={docData.name}
                            className='w-24 h-24 rounded-xl object-cover border-4 border-primary/20 shadow-md bg-primary/10'
                        />
                        <div className='flex-1 min-w-[250px]'>
                            ...
                            <div className='grid grid-cols-2 md:grid-cols-3 gap-4'>
                                <div>
                                    <p className='text-xs text-gray-400 uppercase font-medium'>Degree</p>
                                    <p className='text-sm font-semibold text-gray-700'>{docData.degree}</p>
                                </div>
                                <div>
                                    <p className='text-xs text-gray-400 uppercase font-medium'>Experience</p>
                                    <p className='text-sm font-semibold text-gray-700'>{docData.experience}</p>
                                </div>
                                <div>
                                    <p className='text-xs text-gray-400 uppercase font-medium'>Consultation Fee</p>
                                    <p className='text-sm font-semibold text-primary'>{currencySymbol}{docData.fees}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className='p-6'>
                    ...
                    <div className='grid grid-cols-2 md:grid-cols-4 gap-4 mb-6'>
                        ...
                        <div className='bg-purple-50 rounded-xl p-4'>
                            <p className='text-xs text-purple-600 uppercase font-medium mb-1'>Amount Paid</p>
                            <p className='text-lg font-bold text-purple-800'>{currencySymbol}{amount}</p>
                        </div>
                            ...
                            <div className='flex flex-wrap gap-6'>
                                ...
                                <div>
                                    ...
                                    <p className='text-sm font-semibold text-gray-700 capitalize'>
                                        {appointment.rescheduledBy === 'patient' ? 'You' : 'Doctor'}
                                    </p>
                                </div>
                            </div>
                        ...
                        <div className='p-6'>
                            <p className='text-sm text-gray-500 mb-2'>
                                Cancelled by: <span className='font-semibold capitalize'>
                                    {appointment.cancelledBy === 'patient' ? 'You' : 'Doctor'}
                                </span>
                            </p>
