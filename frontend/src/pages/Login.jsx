...
const Login = () => {
  ...
  // ✅ Flag to prevent useEffect from overriding navigate('/complete-profile')
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      if (state === 'Sign Up') {
        ...
        if (data.success) {
          // ✅ Set flag BEFORE setting token so useEffect skips home navigation
        ...
        if (data.success) {
          ...
          // useEffect will handle navigation to '/' for normal login
        } else {
          toast.error(data.message)
        }
      }
  ...
  // ✅ FIXED: Only navigate home on login — skip if just registered
  useEffect(() => {
    if (token) {
      if (justRegistered.current) {
        // Registration already called navigate('/complete-profile'), skip
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      setOtpLoading(true)
      const { data } = await axios.post(backendUrl + '/api/user/verify-otp', { email: forgotEmail, otp: otpString })
  ...
  const handleResetPassword = async () => {
    ...
    try {
      setOtpLoading(true)
      const otpString = otp.join('')
      const { data } = await axios.post(backendUrl + '/api/user/reset-password', { email: forgotEmail, otp: otpString, newPassword })
      if (data.success) {
        toast.success(data.message)
    ...
    <div className='min-h-[85vh] flex items-center justify-center py-12 px-4'>
      <div className='w-full max-w-md'>
        ...
        <div className='bg-white rounded-2xl shadow-2xl overflow-hidden'>
          ...
          <div className='bg-gradient-to-r from-primary to-blue-600 p-8 text-center'>
            ...
            <p className='text-blue-100'>
              {state === 'Sign Up' ? "Sign up to book your appointment" : "Login to access your account"}
            </p>
          </div>
          ...
          <form onSubmit={onSubmitHandler} className='p-8'>
            <div className='space-y-5'>
              ...
              <div>
                ...
                {state === 'Sign Up' && <p className='text-xs text-gray-500 mt-2'>Password must be at least 8 characters</p>}
                {state === 'Login' && (
              ...
              <div className='text-center pt-4 border-t border-gray-200'>
                {state === "Sign Up" ? (
                  <p className='text-gray-600'>Already have an account?{' '}
                    <button type='button' onClick={() => { setState('Login'); setName(''); setEmail(''); setPassword('') }}
                      className='text-primary font-semibold hover:underline'>Login here</button>
                  </p>
                ) : (
                  <p className='text-gray-600'>Don't have an account?{' '}
                    <button type='button' onClick={() => { setState('Sign Up'); setEmail(''); setPassword('') }}
                      className='text-primary font-semibold hover:underline'>Sign up now</button>
                  </p>
                )}
              </div>
            </div>
        ...
        <p className='text-center text-gray-600 mt-6 text-sm'>
          {state === 'Sign Up' ? 'By signing up, you agree to our Terms of Service' : 'Having trouble? Contact support'}
        </p>
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl">
            ...
            <div className="p-6">
              ...
              {forgotStep === 2 && (
                <div className="space-y-4">
                  <p className="text-gray-600 text-center">We've sent a 6-digit OTP to <strong>{forgotEmail}</strong></p>
                  ...
                  <div>
                    ...
                    <div className='flex justify-center gap-2 sm:gap-3'>
                      {otp.map((digit, index) => (
                        <input key={index} id={`forgot-otp-${index}`} type='text' inputMode='numeric' maxLength='1'
                          value={digit} onChange={(e) => handleOtpChange(e.target, index)}
                  ...
                  <div className='text-center'>
                    {resendTimer > 0 ? (
                      <p className='text-xs text-gray-500'>Resend OTP in <span className='font-semibold text-primary'>{resendTimer}s</span></p>
                    ) : (
                      <p className='text-xs text-gray-800'>Didn't receive OTP?
                        <button onClick={handleResendOTP} disabled={otpLoading}
                          className='text-xs text-primary hover:underline pl-1 font-medium disabled:opacity-50'>Resend OTP</button>
                      </p>
                    )}
                  </div>
              ...
              {forgotStep === 3 && (
                <div className="space-y-4">
                  <div className='text-center mb-2'>
                    ...
                    <p className="text-gray-600">OTP verified! Create a new password for your account</p>
                  </div>
                  ...
                  <div>
                    ...
                    <div className='relative'>
                      <input className='w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-primary focus:outline-none transition-all pr-12'
                        type={showNewPassword ? 'text' : 'password'} onChange={(e) => setNewPassword(e.target.value)}
                  ...
                  <div>
                    ...
                    <div className='relative'>
                      <input className='w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-primary focus:outline-none transition-all pr-12'
                        type={showConfirmPassword ? 'text' : 'password'} onChange={(e) => setConfirmPassword(e.target.value)}
