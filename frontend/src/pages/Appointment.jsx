...
const Appointment = () => {
  ...
  // ✅ FIXED: Always refresh doctor data when component mounts or docId changes
  useEffect(() => {
    getDoctorsData()
  }, [docId])

  // ✅ FIXED: Update docInfo whenever doctors array changes
  useEffect(() => {
  ...
  // ✅ FIXED: Regenerate slots whenever docInfo changes (this will trigger when slots_booked updates)
  useEffect(() => {
    if (docInfo) {
  ...
  return (
    <div>
      ...
      <div className='flex flex-col sm:flex-row gap-4'>
        ...
        <div className='flex-1 border rounded-lg p-8 bg-white -mt-20 sm:mt-0'>
          ...
          <p className='mt-4 text-sm'>{docInfo.about}</p>
      ...
      <div className='sm:ml-10 mt-16'>
        <p className='mb-8 font-medium text-gray-900'>Select Your Slot</p>

        
        <div className='flex gap-3 overflow-x-scroll mb-10'>
              ...
              <div
                ...
                className={`min-w-16 py-6 rounded-full text-center cursor-pointer
                  ${isActive ? 'bg-primary text-white'
                    : hasSlots ? 'border border-gray-800'
        ...
        {slotRows.length > 0 ? (
          slotRows.map((row, rIdx) => (
            <div key={rIdx} className='grid grid-cols-3 sm:grid-cols-7 gap-3 mb-3'>
              {row.map((item, idx) => (
                <button
                  key={idx}
