...
const OTPVerificationModal = ({ 
  ...
  const actionColor = action === 'cancel' ? 'red' : 'blue'
      ...
      <div className="bg-white rounded-2xl p-8 max-w-md w-full shadow-2xl animate-scale-in">
        ...
        <div className='text-center mb-6'>
          <div className={`w-16 h-16 ${action === 'cancel' ? 'bg-red-100' : 'bg-blue-100'} rounded-full flex items-center justify-center mx-auto mb-4`}>
            <span className='text-3xl'>{action === 'cancel' ? '🔐' : '🔒'}</span>
          </div>
        ...
        <div className={`${timeLeft > 0 ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'} border-2 rounded-lg p-3 mb-6 text-center`}>
          <p className='text-sm text-gray-600 mb-1'>
            {timeLeft > 0 ? '⏰ OTP expires in' : '⏱ OTP expired'}
          </p>
          <p className={`text-2xl font-bold ${timeLeft > 0 ? 'text-green-600' : 'text-red-600'}`}>
            {formatTime(timeLeft)}
          </p>
        </div>
          ...
          <div className='flex justify-center gap-2'>
            {otp.map((digit, index) => (
              <input
                ...
                onPaste={handlePaste}
                className='w-12 h-14 text-center text-2xl font-bold border-2 border-gray-300 rounded-lg focus:border-primary focus:outline-none transition-all'
